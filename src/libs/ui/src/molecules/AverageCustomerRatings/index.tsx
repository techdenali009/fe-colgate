...
const calculateAverageRatings = (Reviewproducts: any) => {
  ...
  const total = Reviewproducts.reduce(
    ...
    (acc: any, review: any) => {
      ...
      if (review.ratings.quality?.score)
        acc.quality += review.ratings.quality.score;
...
const AverageCustomerRatings: React.FC<AverageCustomerRatingsProps> = ({
  className,
  headClassName,
}) => {
  ...
  return (
    <div className={`!text-left  lg:my-5 mb-5 lg:text-center   ${className}`}>
      <h3
        className={`text-left p-2.5  font-[16px] text-base  lg:text-center font-SansSerif ${headClassName}`}
      >
