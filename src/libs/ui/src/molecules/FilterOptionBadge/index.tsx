...
import ReviewDropclose from '../../../assets/appSvgs/ReviewDropclose.svg';
...
export const FilterBadge: React.FC<FilterBadgeProps> = ({ children, onRemove, useAltSvg, className, spanclassName, imgclassName }) => {
  return (
    ...
    >
      <span className={`text-blue-700 hover:font-bold text-sm ${spanclassName}`}>{children}</span>
      <button
        onClick={onRemove}
      ...
      >
        {useAltSvg ? (
          <img src={ReviewDropclose} alt="cancel Logo" className={`${imgclassName}`}/>
        ) : (
          <img src={CancelIcon} alt="cancel Logo" className={`w-4 h-4 ${imgclassName}`} />
        )}
      </button>
    </div>
  );
