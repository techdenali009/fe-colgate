...
const ReviewDropdown: React.FC<DropdownProps> = ({
  ...
  return (
    ...
    >
      ...
      {isOpen && (
        <div className={`absolute z-10 bg-white rounded-lg shadow-xl border border-gray-300 ${selectdropclassName}`}>
          {items.map((item, index) => {
            const starCount = parseInt(item, 10);
            const displayText = isRatingDropdown && starCount === 1 ? 'star' : 'stars';
