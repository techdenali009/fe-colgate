...
import CouponForm from '../CouponInput';
...
const PaymentPage: React.FC<paymentProps> = () => {
  ...
  // const totalToPay = +(totalCartPrice + Tax + shippingAmount - discount).toFixed(2);

  const [finalAmount, setFinalAmount] = useState(totalCartPrice + Tax + shippingAmount);
  const handleDiscountApplied = (discountValue: number) => {
    if (isNaN(discountValue) || discountValue < 0) {
      setFinalAmount(totalCartPrice + Tax + shippingAmount);
      setIsCouponApplied(false);
    } else {
      setIsCouponApplied(true);
      setFinalAmount(totalCartPrice + Tax + shippingAmount - discountValue);
    }
  };

  // Compute total to pay dynamically based on whether a coupon was applied
  const totalToPay = isCouponApplied ? finalAmount : (totalCartPrice + Tax + shippingAmount);

  // Update the final amount based on the discounted total
  // const totalToPay = finalAmount;
  const handleCheckout = async () => {
    const createOrderPayload = {
      ...
      taxAmount: Tax,
      totalAmount: totalToPay, // Should reflect the discounted amount
      shippingCost: shippingAmount,
      discount: { discount },
      couponCode: { couponCode },  // Include the applied coupon code
      ...
      <Card className="w-full bg-white relative"> {/* Set relative positioning */}
        ...
        <CardContent className="p-6"> {/* Set relative positioning */}
            ...
            <div className="w-[280px] border-l-2 pl-[20px]">
              ...
              <div className="relative">
                ...
                <ButtonWithTextAndIcon
                  className={`w-full !bg-appTheme !text-white hover:!border-appTheme font-HeroNewSemiBold ${loading ? 'opacity-50 cursor-not-allowed' : ''
                  }`}
          ...
          <CouponForm
            ...
            setDiscountedTotal={handleDiscountApplied}
          />
        </CardContent>
      </Card>
    </>
