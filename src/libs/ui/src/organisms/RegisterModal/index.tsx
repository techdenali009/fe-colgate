...
const RegisterForm: React.FC = () => {
  ...
  const [isConfirmPasswordFieldEmpty, setIsConfirmPasswordFieldEmpty] =
    useState(true);
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    // eslint-disable-next-line  @typescript-eslint/no-explicit-any
      ...
      showErrorToast(
        `Error: ${err.data.message}`, 'top-right'
      );
    }
  };
  const handleFocus = () => {
  ...
  return (
    <>
      ...
      <div className="relative">
        ...
        <div
          className={'flex items-center justify-center min-h-screen bg-gray-100 dark:bg-appdarkcolor transition-all duration-300 '}
        >
          <form
            onSubmit={handleSubmit(onSubmit)}
            className="min-lg:w-[67%] w-[648px] max-md:w-[100%] bg-white p-9 shadow-normal inline-grid !my-16 dark:bg-appModalColor"
          >
            {/* Header */}
            <div className="text-start">
              <p className="text-[32px] mb-[32px] font-HeroNewRegular text-appTextColor">
              Register Your Profile
              </p>
            </div>
            {/* Primary Contact Subtitle */}
            <div className="mb-1  text-left">
              <SubtitleLabel className="font-HeroNewLight !text-appTextColor text-left !text-[16px]">
              Primary Contact
              </SubtitleLabel>
            </div>
            ...
            <div className="mb-2 inline-grid text-start ">
              <div className="flex text-xs font-heroNewLight,font-sans">
                <Label className="text-xs font-HeroNewUltraLight">
                Email Address
                </Label>
                <Label className="ml-0 text-s font-HeroNewLight text-red-600">
                  {' '}
                *
                </Label>
              </div>
            ...
            <div className="mb-2 inline-grid text-start">
              <div className="flex text-xs font-heroNewLight,font-sans">
                ...
                <Label className="ml-0 text-s font-HeroNewLight,font-sans text-red-600">
                  {' '}
                *
                </Label>
              </div>
              ...
              <Controller
                ...
                render={({ field }) => (
                  ...
                  isSubmitted && errors.email
                    ...
                    {watchFirstName && (
                      <FaCheckCircle className="absolute right-2.5 top-[30%] transform -translate-y-1/2 text-appTheme text-base" />
                    )}
                  </div>
                )}
              />
            ...
            <div className="mb-2 inline-grid text-start">
              <div className="flex text-xs font-heroNewLight,font-sans">
                ...
                <Label className="ml-0 text-s font-HeroNewUltraLight,font-sans text-red-600">
                  {' '}
                *
                </Label>
              </div>
              <Controller
                ...
                rules={{
                  required: ValidationForm.Required,
                  validate: (value) =>
                    value === watchPassword || ValidationForm.PasswordNotMatch,
                }}
                render={({ field }) => (
                  <PasswordFeild
                    label=""
...
Confirm Password
            ...
            >
              <label>
              Please send me exclusive offers and updates from PCA SKIN. You
              have the freedom to unsubscribe whenever you wish.
              </label>
            </Checkbox>
            ...
            <Checkbox
              ...
              className={`text-left !m-0 font-HeroNewLight text-[#333333] text-xs pt-[11px] pb-6 !leading-6 rounded-none h-12 p-0  border-[1px] dark:text-appWhiteTheme-opacity-70 ${
                errors[LoginForm.Email] ? 'border-[#595959]' : 'border-none'
              }
            ${
    isSubmitted && (errors[LoginForm.Email] || !checkPolicy)
      ? 'focus:outline-appTheme border-formFieldBorder'
      : 'focus:outline-none'
    }`}
            >
              <label>
              Kindly explore our{' '}
                <a className="text-appTheme text-sm font-HeroNewRegular">
                privacy policy
                </a>{' '}
              to understand how we utilize your information.
            ...
            <div className="flex justify-between items-center w-full">
              ...
              {/* This empty div takes up space to push the login button to the right */}
              <Button
                type="submit"
          ...
          {isLoading && (
            <div className="absolute inset-0 flex justify-center items-center bg-black bg-opacity-50 z-100">
              <AppSpinner containerClassName="!h-16 !w-16" />
            </div>
          )}
        </div>
      </div>
